...
class Template
{
    ...
    private function compile()
    {
         ...
         * {foreach from=$aa item=v} => <?php foreach ($aa as $v) { ?>
         * {foreach from=$aa key=k item=v} => <?php foreach ($aa as $k => $v) { ?>
         * {/foreach} => <?php } ?>
        ...
        $p = 'foreach\s+from\s?=\s?' . $vn . '\s+item\s?=\s*' . $vs . ';?\s*';
        $a = preg_replace($lt . $p . $gt, '<?php foreach (\\1 as \$\\2) { ?>', $a);
        
        $p = 'foreach\s+from\s?=\s?' . $vn . '\s+key\s?=\s?' . $vs . '\s+item\s?=\s?' . $vs . ';?\s*';
        $a = preg_replace($lt . $p . $gt, '<?php foreach (\\1 as \$\\2 => \$\\3) { ?>', $a);
        
        $p = 'foreach\s+from\s?=\s?' . $vn . '\s+item\s?=\s?' . $vs . '\s+key\s?=\s?' . $vs . ';?\s*';
        $a = preg_replace($lt . $p . $gt, '<?php foreach (\\1 as \$\\3 => \$\\2) { ?>', $a);
