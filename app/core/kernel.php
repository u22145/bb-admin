...
<body>
    ...
    @file_put_contents(ERRLOG_PATH . '/kernal_' . date("Ymd") . '.log',
                date('Y-m-d H:i:s', time()) . ' Kernal Errors '. $type . " : " . json_encode($data) . " msg: \n $msg \n",
                FILE_APPEND);

    // AJAX错误提示
    IS_POST && ajax_return(0, $msg);
    // 页面显示错误
    $html .= "</div>\n</body>\n</html>";
    exit($html);
}
...
function ajax_return($status = 1, $msg = '', $data = [])
{
    ...
    @file_put_contents(ERRLOG_PATH . "/ajax_return_" . date("Ymd") . ".log", json_encode($return, JSON_UNESCAPED_UNICODE) . PHP_EOL);
...
spl_autoload_register(function($class_name) {
    $spl_autoload_register_find = false;
    $dirs = ['controller', 'library', 'model', 'consts'];
    foreach ($dirs as $dir) {
        $file_path = APPPATH . $dir. '/' . $class_name . '.php';
        if (file_exists($file_path)) {
            $spl_autoload_register_find = true;
            include $file_path;
            break;
        }
    }
    if (!$spl_autoload_register_find) {
    ...
    } elseif (file_exists(APPPATH . 'model/' . $class_name . '.php')) {
        include APPPATH . 'model/' . $class_name . '.php';
    } else {
        show_error(997, ['title' => "The class <i>$class_name</i> does not exist."]);
    }*/
});
